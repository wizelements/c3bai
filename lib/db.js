...
// ─── Leads (Phase 2/3 revenue funnel) ────────────────────────────────────────

export async function insertLead({ email, firstName, source = 'lead_magnet', sourceUrl = '' }) {
  const db = getClient();
  if (!db) {
    console.warn('[db] Turso not configured; lead not persisted.');
    return { success: false, reason: 'not_configured' };
  }
  ...
  try {
    await db.execute({
      sql: `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      args: [id, email, firstName || '', source, sourceUrl || '', 'subscribed', 0, now, now],
    });
    return { success: true, id };
  } catch (error) {
    console.error('[db] insertLead failed:', error);
    return { success: false, error: error.message };
  }
}

export async function getLeads(status = null, limit = 200) {
  const db = getClient();
  if (!db) return { success: false, reason: 'not_configured', data: [] };
  ...
  try {
    ...
    console.error('[db] getLeads failed:', error);
    return { success: false, error: error.message, data: [] };
  }
}
...
export async function getLeadById(id) {
  ...
  try {
    ...
    console.error('[db] getLeadById failed:', error);
    return { success: false, error: error.message, data: null };
  }
}

export async function updateLeadFollowUp(id, draft) {
  const db = getClient();
  if (!db) return { success: false, reason: 'not_configured' };
  ...
  try {
    ...
    console.error('[db] updateLeadFollowUp failed:', error);
    return { success: false, error: error.message };
  }
}

export async function markLeadFollowUpSent(id) {
  const db = getClient();
  if (!db) return { success: false, reason: 'not_configured' };
  ...
  try {
    ...
    console.error('[db] markLeadFollowUpSent failed:', error);
    return { success: false, error: error.message };
  }
}

export async function markLeadDownloaded(id) {
  const db = getClient();
  if (!db) return { success: false, reason: 'not_configured' };

  try {
    await db.execute({
      sql: 'UPDATE leads SET downloaded = 1, updated_at = ? WHERE id = ?',
      args: [new Date().toISOString(), id],
    });
    return { success: true };
  } catch (error) {
    console.error('[db] markLeadDownloaded failed:', error);
    return { success: false, error: error.message };
  }
}
